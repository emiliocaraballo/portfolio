...
class ProjectController{
    ...
    public create= async (req: Request, res: Response): Promise<void> => {
        ...
        const { name,mail,subject,message }: IProject = req.body;
        ...
        try {
            ...
            const response: IQueryResponse =await projectModel.create(req.body);
            if(response.ok){
                var to="emiliocaraballo9810@gmail.com";
                var replay="";
                mailer.smart_main(String(process.env.NOMBRE_PROYECTO),'Proyecto - '+process.env.NOMBRE_PROYECTO,`<p>${name}<br>${mail}<br>${subject}<br>${message}</p>`,to,replay);
                res.status(200).json({
                    code:1,
                    data: response.data,
